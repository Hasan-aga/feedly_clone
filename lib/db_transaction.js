...
export async function updateFeedArticles(feedID, articles, client) {
  ...
  console.debug("updating articles of feed ", feedID);
  try {
    // start transaction
    const timestamp = new Date();
    ...
    console.debug("finished updating articles of feed ", feedID);
  } catch (error) {
    console.error("error while updating feed");
    throw new Error(`failed to process query, ${error}`);
  }
}
