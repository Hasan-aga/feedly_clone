...
export class Controller {
  ...
  async addNewFeed(url, category) {
    ...
    try {
      ...
      await addCategory(this.userid, feedID, correctedCategory, this.client);
  ...
  async getMyFeeds() {
    try {
      ...
      console.log("Returned from getmyfeeds");
      // Start the checkAndUpdate process after 1 second.
      setTimeout(() => {
        this.checkAndUpdate().catch((err) => {
          console.error("Error during checkAndUpdate:", err);
        });
      }, 1000);
    }
  }
  ...
  async checkAndUpdate() {
    ...
    try {
      for (const feed of this.#GlobalResults) {
        // check if feed needs updating articles
        if (needsUpdate(feed.lastupdated)) {
          console.log("updating this feed", feed);
